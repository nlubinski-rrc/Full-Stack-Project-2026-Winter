...
function MovieSearchBar({ addItemToList }: SearchBarProps) {
    ...
    function searchData(e?: React.FormEvent) {
        ...
        for (const movie of movieData["results"]) {
            if (movie.title.toLowerCase().includes(searchQuery.toLowerCase()) && recordsPrinted < 4) {
                recordsPrinted += 1
                movieResults.push(
                    <div key={movie.id}>
                        <div id="results" ><MovieCard movie={[movie.title, movie.vote_average.toString(), movie.overview]}/></div>
                        <button className="add-to-watchlist-button" onClick={() => addToWatchlist(movie.id, movie.title)}>Add to Watchlist</button>
                    </div>
                )
            }
        }
    ...
    return (
        <form>
            <div id="input-fields">
               <input type="search" placeholder="Seach Movies" onChange={(e) => updateResults(e.target.value)} />
            </div>
